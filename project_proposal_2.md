...
Field macro-accuracy ต่อหลักสูตร: AIT 0.991 | DSBA 0.979 | IT 0.945 | BIT 0.953 (เฉลี่ย 0.967)
...
**บั๊กที่ evaluation ตรวจเจอและแก้แล้ว:**

1. regex สกัดชื่อวิชาทำเลขลำดับหาย ("แคลคูลัส 1" → "แคลคูลัส") และดูดเลขหน่วยกิตเข้าชื่ออังกฤษ ("LINEAR ALGEBRA 3") — หลังแก้ ชื่ออังกฤษดีขึ้นจาก 0.213 → 0.926
2. ชั้นปี/ภาคการศึกษาใช้ marker ตัวสุดท้ายของหน้าเป็นค่าของทั้งหน้า แต่พบ 52 หน้าที่มี marker หลายค่า (ปี 3 เทอม 1 กับ ปี 3 เทอม 2 อยู่หน้าเดียวกัน) — แก้เป็นจับคู่รายวิชากับ marker ที่อยู่ก่อนรหัสวิชาใกล้สุด
3. วิชาที่ถูกสกัดจากหน้าคำอธิบายรายวิชา (ไม่มี marker) ไม่เคยได้ชั้นปี เพราะตารางแผนบางเล่มไม่มีหน่วยกิตรูป `3(3-0-6)` ให้ regex จับ — แก้โดยเพิ่ม pass เติมชั้นปีจาก majority vote ของทุกหน้าที่พบ "รหัสวิชา + marker"
4. ตาราง `plan_slot` ว่างเปล่าทั้งที่รายงานว่าเพิ่ม 2,149 แถว เพราะ `provenance_id` เป็น NOT NULL แล้ว `INSERT OR IGNORE` ข้ามเงียบ ส่วนตัวนับบวกทุกรอบ — แก้ทั้งการส่ง provenance และเปลี่ยนไปนับจาก `rowcount` จริง

ผลรวมของข้อ 2–4: ชั้นปี 0.903 → 0.979, ภาคการศึกษา 0.903 → 1.000, IT field macro 0.895 → 0.945
...
|---|---:|---|
| citation page precision | **0.632** | หน้าที่อ้าง อยู่ในชุดหน้าหลักฐานจริงกี่ % |
| citation page recall | **0.580** | หน้าหลักฐานจริง ถูกอ้างถึงกี่ % |

จำกัดหน้าหลักฐานที่ 10 หน้าต่อคำถาม เพื่อให้เพดาน recall ตีความได้ (แสดงคอลัมน์เพดานในรายงาน)

**การปรับปรุงที่ทำเพื่อยกตัวเลขนี้** (จากค่าเริ่มต้น 0.300 → 0.632):
...
5. **แยกคำถามที่ชี้วิชาเดียว** — ถาม "วิชาระบบข้อมูลมหัต ชื่อภาษาอังกฤษว่าอะไร" เคยได้ candidate 12 วิชา เพราะ (ก) `clean_topic` ไม่ตัด "ชื่อภาษาอังกฤษว่าอะไร" ออก ทำให้ keyword แตกเป็น "ภาษา/อังกฤษ" แล้วดึงวิชาภาษาอังกฤษพื้นฐานมา (ข) ไม่มีการตรวจว่าคำถามชี้วิชาเดียว เพิ่ม `find_exact_course` (ชื่อวิชาตรงกับ topic ทั้งชื่อ) พร้อม guard `wants_multiple` กันคำถามพหูพจน์ ("มีอะไรบ้าง", "กี่วิชา") ไม่ให้เข้าเส้นทางนี้ — ยกค่าจาก 0.574 → 0.632 (E5 จาก 0.125 → **1.000**)
