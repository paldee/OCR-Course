...
|---|---:|---|
| citation page precision | 0.300 | หน้าที่อ้าง อยู่ในชุดหน้าหลักฐานจริงกี่ % |
| citation page recall | 0.279 | หน้าหลักฐานจริง ถูกอ้างถึงกี่ % |

จำกัดหน้าหลักฐานที่ 10 หน้าต่อคำถาม เท่ากับจำนวน citation ที่ระบบคืน เพื่อให้เพดาน recall = 1.0 และตีความค่าได้

**ตัวเลขนี้ต่ำและเป็นจุดที่ต้องปรับปรุงต่อ** — แปลว่า retrieval ยังดึงหน้าที่ไม่ตรงประเด็นมาด้วยประมาณ 7 จาก 10 หน้า แนวทางแก้: ลดจำนวน citation ที่คืน (คืนเฉพาะหน้าที่คะแนนสูงจริง) และเพิ่ม re-ranking หลัง hybrid retrieval

**บั๊กที่พบตอนสร้างการวัดนี้:** ฟิลด์ `citations[].document_id` เคยเก็บ `chunk_id` ไม่ใช่ document_id จริง ทำให้ตรวจย้อนกลับไปหน้าใน PDF ไม่ได้และวัด citation ไม่ได้เลย — แก้แล้วโดยให้ `retriever`/`semantic_retriever` ส่ง `document_id` มาด้วย
