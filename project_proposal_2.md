...
|---|---|---|---|
| 1 | หลักสูตร DSBA 2565 ต้องเรียนครบกี่หน่วยกิตจึงจะสำเร็จการศึกษา? | 132 หน่วยกิต | หน้า 7 หัวข้อ "โครงสร้างหลักสูตร" |
| 2 | วิชา 06026212 (Data Warehousing) เปิดสอนปีที่เท่าไร เทอมอะไร? | ปีที่ 3 ภาคการศึกษาที่ 1 | หน้า 15 ตารางแผนการเรียน |
| 3 | หลักสูตร IT 2565 มีกี่แขนงวิชา? | **3 แขนง** (Software Engineering, Network and System Technology, Multimedia and Game Development) — *แก้จากเฉลยเดิมที่เขียน 4 แขนง ซึ่งไม่ตรงเอกสาร* | หน้า 372 "ให้ผ่าน 1 แขนงวิชา จาก 3 แขนงวิชา" (หน้า 20 เรียกเป็น "โมดูลวิชาสาขา" 3 โมดูล) |
...
| 2 | หมวดวิชาเฉพาะเลือกของ DSBA เก็บกี่หน่วยกิต และต้องเรียนตอนไหน? | ไม่น้อยกว่า **12 หน่วยกิต** (*แก้จากเฉลยเดิม 6 หน่วยกิต* — หน้า 15 โครงสร้างระบุ "กลุ่มวิชาชีพเลือก 12 หน่วยกิต"; เลข 6 ที่ตอบคือ "กลุ่มวิชาการศึกษาทางเลือก" ซึ่งเป็นคนละหมวด) เริ่มลงตามแผนที่ **ปี 3 ภาค 1** (แผนการเรียนหน้า 27-29 มี ELECTIVE IN DATA SCIENCE/STATISTICAL/DATA ENGINEERING เริ่มปรากฏในปี 3) | หน้า 15 (โครงสร้าง) + หน้า 27-29 (แผนเรียนปี 3-4) |
...
| 4 | วิชาที่มีในหลักสูตร DSBA 2560 แต่ไม่มีในหลักสูตร 2565 มีอะไรบ้าง? | **เฉลย (คำนวณจากฐานข้อมูลจริง — set-difference ตามชื่อวิชา):** DSBA 2560 มี 71 วิชา, 2565 มี 75 วิชา โดย **48 วิชาของ 2560 ไม่มีชื่อตรงใน 2565** เพราะปี 2565 ปฏิรูปหลักสูตรใหญ่ (เปลี่ยนชุดรหัสวิชาและชื่อเกือบทั้งหมด) ตัวอย่างวิชาที่หายไป เช่น พื้นฐานทางด้านเทคโนโลยีสารสนเทศ (06026100), เทคโนโลยีเว็บ (06026109), การวิเคราะห์ข้อมูลเชิงธุรกิจ (06026116), ระบบฐานข้อมูลขั้นสูง (06026145), การจัดการนวัตกรรมและการเปลี่ยนแปลง (06026152), เศรษฐศาสตร์เทคโนโลยีสารสนเทศ (06026155) และกลุ่มวิชาศึกษาทั่วไป/ธุรกิจอีกหลายวิชา *(สำหรับการวัดผล ควรใช้จำนวน 48 วิชาเป็น ground truth เชิงปริมาณ แทนการระบุตัวอย่างกว้าง ๆ)* | เทียบตารางรายวิชา DSBA 2560 vs 2565 |
...
| 2 | เรียนวิชา Database ปี 4 ได้ไหม? | **ได้** — *แก้จากเฉลยเดิมที่ตอบ "ไม่ได้"* เอกสารหลักสูตรระบุ "แผนการศึกษา" ที่แนะนำ ไม่ใช่ข้อห้าม และไม่มีข้อความห้ามลงวิชาบังคับช้ากว่าแผน (กรณีถอน/ตกต้องลงใหม่ได้) นอกจากนี้ยังมีวิชาฐานข้อมูลที่เปิดสอนปี 4 อยู่จริง คือ 06026246 ระบบฐานข้อมูลแบบกระจาย (ปี 4 ภาค 1, ไม่มีวิชาบังคับก่อน) ส่วน 06066300 Database System Concepts นั้นแผนวางไว้ปี 2 ภาค 1 | หน้า 15 แผนเรียน + หน้า 323 คำอธิบายรายวิชา |
...
| 6 | ถ้าจะจบใน 3.5 ปี ตรวจว่าแผนเรียนครบเงื่อนไขจบหรือไม่? | ⚠️ **คำถามกำกวม ควรเขียนใหม่** — ข้อนี้ไม่ได้ให้ "แผนเรียน" มาด้วย ระบบจึงไม่มีอะไรให้ตรวจ ตอบได้แค่ลอกเกณฑ์จบมาแสดง ซึ่งซ้ำกับ H1 ถ้าต้องการวัดความสามารถตรวจแผน ต้องแนบแผนสมมติมาในคำถาม เช่น "ถ้าลงวิชาตามนี้ ... ครบเงื่อนไขจบหรือไม่" เกณฑ์จบที่ถูกคือ **132 หน่วยกิต** (ไม่ใช่ 126 ตามที่เขียนไว้เดิม), วิชาบังคับครบ, วิชาเลือกเฉพาะด้าน 12 หน่วยกิต, เลือกเสรี 6 หน่วยกิต, prerequisite ผ่านก่อนลงวิชาถัดไป | หน้า 7 (เกณฑ์จบ) + หน้า 14-16 (แผน) |
...
#### ระดับที่ 1.5 — CER (วัดคุณภาพ Tesseract โดยตรง)

**สคริปต์:** `python -m katrag.eval.ocr_cer_eval` → รายงาน `artifacts/ocr_cer_report.md`

ตัวเลขในหัวข้อ 6.1 ระดับที่ 2 วัดฟิลด์รายวิชาเทียบ GT ของอาจารย์ได้ แต่รายวิชาที่จับคู่ GT ได้ทั้ง 272 รายการมาจากหน้า **text layer ทั้งหมด** (GT คือตารางแผนการเรียน ซึ่งใน 4 เล่มนั้นเป็นหน้าที่ PDF ฝังข้อความมาแล้ว) ตัวเลขชุดนั้นจึงไม่ได้วัด OCR เลย จึงต้องมีการวัดอีกชุดที่วัด Tesseract ตรง ๆ

**วิธีวัด (paired-page protocol)** — ไม่มี ground truth ระดับข้อความที่คนพิมพ์เอง จึงใช้หน้าที่มี text layer เป็น reference แล้วส่งภาพของ *หน้าเดียวกัน* เข้า Tesseract ด้วย DPI และ argument ชุดเดียวกับ pipeline จริง (300 DPI, `-l tha+eng --psm 6`) ชุดวัด 14 เล่ม / 56 หน้า / 103,870 อักขระ (สุ่มด้วย seed คงที่ รันซ้ำได้ผลเดิม)

| ตัวชี้วัด | ค่า | ความหมาย |
|---|---:|---|
| CER ไม่นับช่องว่าง | 0.205 | **ขอบบน** — รวมความผิดของลำดับการอ่านไว้ด้วย |
| **line-match CER** | **0.045** | **คุณภาพการรู้จำอักขระของ Tesseract** (ค้นแต่ละบรรทัดในหน้าทั้งหน้าก่อนเทียบ) |
| bag CER | 0.038 | ขอบล่าง (เทียบอักขระไม่สนลำดับ การอ่านผิดหักกลบกันได้) |
| รหัสวิชา 8 หลัก อ่านตรง | 0.935 | 86/92 รหัส — สร้างเกินมา 0 รหัส |
| เวลา OCR | 2,454 ms/หน้า | เฉลี่ย |

สามค่าแรกใช้ตัวหารเดียวกัน (อักขระอ้างอิงหลังตัดช่องว่าง) จึงเทียบกันตรง ๆ ได้

**ข้อค้นพบสำคัญ — error ส่วนใหญ่ไม่ใช่การอ่านตัวอักษรผิด แต่เป็นลำดับการอ่าน:**

| ชนิดหน้า | จำนวนหน้า | CER ไม่นับช่องว่าง | line-match CER | ส่วนที่มาจากลำดับ |
|---|---:|---:|---:|---:|
| ตาราง | 17 | 0.472 | 0.074 | +0.398 |
| ข้อความบรรยาย | 39 | 0.124 | 0.037 | +0.088 |

มคอ.2 มีหน้าตารางเยอะ (ตารางเปรียบเทียบหลักสูตรฉบับปรับปรุง, ตาราง PLO mapping, ประวัติอาจารย์) `--psm 6` มองหน้าเป็นบล็อกข้อความเดียว Tesseract จึงไล่อ่านทีละคอลัมน์ ขณะที่ text layer ไล่ทีละแถว — ข้อความเดียวกันแต่เรียงต่างกันจึงถูกนับเป็น error เต็ม ๆ

**สิ่งที่ข้อค้นพบนี้เปลี่ยน:**

- คุณภาพการรู้จำอักขระของ Tesseract อยู่ที่ **0.045** ใกล้เกณฑ์ 0.05 มาก ไม่ใช่ 0.20 อย่างที่ CER ดิบชี้ — **การเปลี่ยน OCR engine ไม่ใช่งานที่ให้ผลตอบแทนสูงสุด**
- สิ่งที่ควรทำก่อนคือ **layout analysis** (ตรวจหาคอลัมน์/เซลล์แล้วส่ง OCR ทีละบล็อก) ซึ่งใช้ Tesseract เดิมได้เลย
- Typhoon OCR ยังน่าทดลองอยู่ เพราะเป็น VLM ที่คืนผลเป็น markdown จึงมีโอกาสรักษาโครงตารางได้ดีกว่า แต่เหตุผลคือ "จัดการ layout" ไม่ใช่ "อ่านตัวอักษรแม่นกว่า"

**ข้อจำกัดที่ต้องระบุ:** reference คือ text layer ไม่ใช่คนพิมพ์ ตัวเลขชุดนี้จึงเป็น proxy ของ CER จริง และวัดจาก 56 หน้า ไม่ใช่ทั้ง 3,689 หน้า
...
Field macro-accuracy ต่อหลักสูตร (7 ฟิลด์: ชื่อไทย/อังกฤษ, หน่วยกิต, ชั้นปี, ภาค, หมวดวิชา, ประเภทวิชา):

DSBA 0.971 | AIT 0.963 | IT 0.941 | BIT 0.935

**category/type เป็นฟิลด์ใหม่ที่เพิ่งสกัดได้** (จากเดิม NULL 100% ทุกรายการ):
- **category** (หมวดวิชาเฉพาะ/ศึกษาทั่วไป/เลือกเสรี): AIT 85% | BIT 87% | DSBA 95%
- **type** (บังคับ/เลือก): AIT 93% | BIT 91% | DSBA 96% *(วัดเฉพาะวิชาที่อยู่ใน GT)*

วิธีสกัด: จับ header หมวดวิชา/วิชาบังคับ/วิชาเลือก ตามตำแหน่งในหน้า แล้ว carry-forward สำหรับ category ส่วน type ใช้สัญญาณจากหน้าแผนการเรียนเป็นหลัก (หน้าที่มี marker ปี/ภาค ถ้าไม่มี header วิชาเลือก → วิชาบังคับ) เพื่อกัน "วิชาเลือก" จากหน้าโครงสร้างรั่วไปทับ

> **ข้อจำกัดของ type ที่ต้องระบุตอนนำเสนอ (CHK1/CHK7 จับได้):** ตัวเลข type 93%/91% ข้างต้นวัดจากวิชาที่อยู่ใน GT เท่านั้น แต่เมื่อรวมทั้งเล่ม พบว่า AIT ถูกจัดเป็นวิชาบังคับ 213 วิชา (หน่วยกิตบังคับรวม 627 > เกณฑ์จบ 120) และ IT 114 วิชา ซึ่งเป็นไปไม่ได้ — เกิดจาก course extraction ดูดวิชาเลือก/วิชาศึกษาทั่วไปเข้ามาเกินจำนวนจริง (AIT มี 366 วิชาในฐาน ทั้งที่หลักสูตรตรีมีราว 120-150) **เคยลองแก้ type แล้วแต่ย้อนกลับ เพราะทำ field accuracy ตกจาก 0.97 เหลือ 0.89** ต้นเหตุจริงอยู่ที่ course extraction ซึ่งต้องขยาย GT ให้ครอบครบก่อนจึงจะแก้ได้ปลอดภัย (บันทึกใน `docs/removed_subsystems.md` ภาคผนวก 2)

**Prerequisite extraction** (ใหม่):
- IT: 12/16 ถูกต้อง (75%) จากเดิม 0/16
- BIT: 2/3 ถูกต้อง
- รวม: **88 วิชาจาก 1,419 มีข้อมูล prerequisite ที่ถูกตัว** (จากเดิม 54 แต่จับผิดทิศ)
- วิธี: ตัดข้อความเป็นบล็อกตรงรหัสวิชาที่มี credit ตามหลัง แล้วหา "วิชาบังคับก่อน" ในบล็อก (รวม chunk ต่อ version เพื่อไม่ให้บล็อกถูกตัดขาดกลางคัน)
- **บั๊กที่ CHK5 (ดูหัวข้อ 6.4) จับได้และแก้แล้ว:** ตัวตรวจ "ไม่มี/None" หลังคำว่า "วิชาบังคับก่อน" ใช้ regex ปิดท้ายด้วย `\b` ซึ่งใช้กับคำไทยไม่ได้ (คำว่า "ไม่มี" ลงท้ายด้วยสระ ี ที่ Python ไม่นับเป็น word character) ทำให้ตัวตรวจฝั่งไทยไม่ทำงานเลย และไปเก็บรหัสวิชาถัดไปเป็น prerequisite ผิด — แก้เป็น negative lookahead ตัดออก 14 รายการที่เอกสารเขียนว่า "ไม่มี" (ยืนยันกับเอกสารต้นฉบับทุกตัว, ไม่มี prerequisite จริงหายไป)
...
เพื่อวินิจฉัยได้ว่าพลาดตรงไหน ระบบวัดสองด้านแยกกัน:

| มิติ | Text-to-SQL ทั่วไป | ระบบนี้ (RAG) วัดอะไรแทน |
|---|---|---|
| 1. กลไกทำงาน | SQL รันผ่าน | **Retrieval hit** — ดึงหน้าหลักฐานที่อยู่ใน gold_set มาได้ไหม |
| 2. คำตอบถูก | ตรงเฉลย | **Answer accuracy** — auto-screen + ผู้ตรวจกา |

ระบบนี้ไม่ได้ให้ LLM สร้าง SQL (เส้นทาง structured ใช้ SQL ที่เขียนตายตัว จึงรันผ่านเสมอ 100% ไม่มีความหมาย) จึงวัด retrieval hit แทน — สะท้อนว่ากลไกค้นหลักฐานทำงานถูกจริงไหม

**ประโยชน์:** คำถามที่ผ่านมิติ 1 แต่ตกมิติ 2 ชี้ว่าปัญหาอยู่ที่ LLM/prompt ไม่ใช่ retrieval — เช่น H5 (retrieval=hit แต่ตอบ "6 หน่วยกิต" แทน 12) ตรงกับบั๊ก retrieval เชิงเนื้อหาที่บันทึกไว้

**ผลล่าสุด:**

| มิติ | ผล | หมายเหตุ |
|---|---|---|
| 1. Retrieval hit | **15/15** ของข้อที่วัดได้ | อีก 4 ข้อยังไม่มีหน้าหลักฐานใน gold_set (ตัดออกจากตัวหาร) |
| 2. Answer accuracy (auto-screen) | **18/19** | H5 ตอบหน่วยกิตผิด (บั๊ก retrieval เชิงเนื้อหา) — ถูกต้องที่ถูก flag |

auto-screening เป็นการคัดกรองเบื้องต้นด้วย substring คะแนน accuracy จริงต้องให้ผู้ตรวจกาช่อง "คำตอบถูก AND อ้างอิงถูก" ในตารางท้ายรายงาน `artifacts/qa_eval_report.md`
...
**การปรับปรุงที่ทำเพื่อยกตัวเลขนี้** (จากค่าเริ่มต้น 0.300 → 0.599):
...
### 6.4 การตรวจความสอดคล้องของข้อมูล (Consistency Check — CHK1-CHK7)

**สคริปต์:** `python -m katrag.cli consistency` → รายงาน `artifacts/consistency_report.md`

การวัดในหัวข้อ 6.1-6.3 ต้องมีเฉลย (teacher GT มีแค่ 4 หลักสูตร) ส่วนชั้นนี้ตรวจว่าข้อมูลที่สกัดมา **สอดคล้องกันเอง** หรือไม่ จึง**ไม่ต้องใช้เฉลย** และครอบทั้ง 13 เวอร์ชันหลักสูตร ทำหน้าที่เป็น regression guard ทุกครั้งที่แก้ ingest

หลักการ: เอกสารเขียนข้อมูลชุดเดียวกันไว้สองที่ ต้องตรงกัน หรือค่าต้องอยู่ในกรอบที่โครงสร้างหลักสูตรอนุญาต

| รหัส | กฎ | จับความผิดแบบไหน | ระดับ | ผลบนข้อมูลจริง |
|---|---|---|---|---|
| CHK1 | หน่วยกิตบังคับรวม ≤ เกณฑ์จบ | แผนตกหล่น/วิชาเลือกถูกจัดเป็นบังคับ | warning | ตรวจ 7 → พบ 3 (AIT/IT type) |
| CHK2 | ทุกรหัสในแผนมีคำอธิบายรายวิชา | รหัสผิด/คำอธิบายไม่ครบ | warning | ตรวจ 322 → พบ 3 |
| CHK3 | รหัสวิชาเป็นเลข 8 หลัก | OCR อ่านตัวอักษรปนในรหัส | error | ตรวจ **1,419 → ผ่าน** |
| CHK4 | หน่วยกิตในแผน = ในคำอธิบาย | เลขหน่วยกิตผิดที่ใดที่หนึ่ง | error | ตรวจ **670 → ผ่าน** |
| CHK5 | วิชาบังคับก่อนอยู่ภาคที่มาก่อน | prerequisite/ปี/ภาคผิด | error | ตรวจ 48 → พบ 2 (เป็น warning) |
| CHK6 | ไม่มีวิชาซ้ำในภาคเดียวกัน | ตารางถูกอ่านซ้ำสองรอบ | error | ตรวจ 322 → ผ่าน |
| CHK7 | หน่วยกิตต่อภาค 9-22 | บางวิชาตกหล่น/มีวิชาปน | warning | ตรวจ 45 → พบ 6 (AIT/IT type) |

**ผลรวม: error 0 / warning 14** — ผ่านเกณฑ์ (warning ต้องใช้คนอ่านประกอบ ไม่ใช่ข้อมูลผิดแน่)

**ตัวเลขที่ใช้อ้างคุณภาพได้โดยไม่ต้องมีเฉลย:**
- **CHK4:** หน่วยกิตของ **670 รายวิชา** ที่เอกสารพิมพ์ไว้สองที่ (ตารางแผน + หน้าคำอธิบาย) ตรงกันทั้งหมด
- **CHK3:** รหัสวิชาทั้ง **1,419 ตัว** เป็นเลข 8 หลักสะอาด ไม่มีตัวอักษรจาก OCR ปน
- **CHK6:** ไม่มีวิชาซ้ำในภาคเดียวกันเลย = ไม่มีตารางถูกอ่านซ้ำ

**บั๊กที่ CHK ชุดนี้จับได้ในรอบเดียว:** CHK5 จับ false positive ของ prerequisite 14 รายการ (บั๊ก `\b` คำไทย — ดูหัวข้อ 6.1) และ CHK1/CHK7 ชี้บั๊ก type/course extraction ของ AIT/IT ที่ซ่อนอยู่ก่อนหน้า — แสดงว่าการมีชั้นวัดผลหลายชั้นช่วยกันจับสิ่งที่ GT อย่างเดียวมองไม่เห็น
