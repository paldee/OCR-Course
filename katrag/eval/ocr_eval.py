...
def build_report(
    ...
    A("")
    if tot_ocr_src == 0:
        A("**ขอบเขตของตัวเลขในหัวข้อ 2:** รายวิชาที่จับคู่กับ GT ได้ทั้ง "
          f"{tot_tl} รายการ มาจากหน้า **text layer** ทั้งหมด (จากหน้า OCR = 0)")
        A("เพราะ GT ของอาจารย์คือ *ตารางแผนการเรียน* ซึ่งในทั้ง 4 เล่มอยู่ในหน้าที่ PDF ฝังข้อความมาแล้ว")
        A("")
        A("ดังนั้นต้องตีความให้ถูก:")
        A("")
        A("- ตัวเลข field accuracy ข้างต้นวัด **คุณภาพ text layer + ขั้นตอน parsing** ")
        A("  ไม่ได้วัดคุณภาพของ OCR")
        A("- การเปลี่ยน OCR engine จะ **ไม่ทำให้ตัวเลขชุดนี้เปลี่ยน** เพราะข้อมูลที่วัดไม่ผ่าน OCR")
        A("- แต่ **ไม่ได้หมายความว่า OCR ไม่สำคัญ** — OCR เป็นแหล่งของรายวิชา 600 จาก 1,419 รายการ "
          "(42%) และ chunk 3,094 จาก 11,587 (27%) ทั่วทั้งฐานข้อมูล ")
        A("  ส่วนนั้นเป็นวิชาเลือก วิชาศึกษาทั่วไป และคำอธิบายรายวิชา ซึ่ง GT ไม่ครอบคลุม จึงวัดตรง ๆ ไม่ได้")
        A("")
        A("รายละเอียดการแยก OCR / text layer ทั้งระบบดูที่ `artifacts/extraction_method_report.md`")
        A("")
