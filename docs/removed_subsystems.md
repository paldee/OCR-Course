...
ทั้งสองข้อต้องแก้ที่ `ingest/chunker.py` (ไม่ตัดกลางรายการ/กลางประโยค) และตรวจน้ำหนัก
fusion ใน `query/hybrid_retriever.py` — เป็นงานที่กระทบ index ทั้งชุด จึงยังไม่ทำในรอบนี้
