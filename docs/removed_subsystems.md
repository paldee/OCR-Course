...
## ภาคผนวก 3: งานแก้ retrieval รอบ ก.ย. 2569 (neighbor-chunk expansion)

### ตรึง baseline ก่อนแตะโค้ด

รันชุดวัดผลสามชุดยืนยันตัวเลขก่อนเริ่ม (commit baseline artifacts):
- consistency: error 0 / warning 14
- ocr_eval: field_macro DSBA 0.971 / AIT 0.963 / IT 0.941 / BIT 0.935
- qa_eval: retrieval hit 15/15 วัดได้, auto_pass 18/19 (H5 ตก)

### ปิดช่องว่างการวัด (gold_set)

คำถาม 4 ข้อ (E2, E3, H1, H4) เคยตกลงไปที่ `no_gold` เพราะ `build_gold_set`
derive หน้าหลักฐานจาก `item.check` เท่านั้น และ 4 ข้อนี้มี check เป็นตัวเลขล้วน
(`3`, `2560`) ที่ตัวกรอง length/digit ตัดทิ้ง จึงไม่เหลือ token ไป derive

แก้โดยเพิ่ม "ชั้น 3" ใน `expected_pages()` — เมื่อชั้น 1-2 หาไม่ได้ ให้ดึง keyword
จาก *ตัวคำถาม* ด้วย `extract_keywords` (ตัวเดียวกับที่ retrieval ใช้จริง) ไม่เป็น
self-check เพราะคำถามคือ input ของผู้ใช้ ไม่ใช่คำตอบของระบบ ผลคือ retrieval hit
วัดได้ 18/19 (E2/E3 ได้ precision 1.0) ส่วน H4 กลายเป็น **miss** ที่มองเห็นได้ (เดิม
ซ่อนใน no_gold) — คำตอบต้องอ้างหลักสูตรเก่า 2560 แต่ `_resolve_version` เลือกเฉพาะ
`edition_status='current'` gold จึงชี้ผิดเวอร์ชัน เป็นข้อจำกัด cross-version จริง
ที่การวัดเพิ่งเผยให้เห็น ไม่ใช่ regression

### H5 — ต้นเหตุจริงคือ context ถูกตัด ไม่ใช่ retrieval ดึงไม่เจอ

"หมวดวิชาเฉพาะเลือกเก็บกี่หน่วยกิต" (DSBA) ตอบ "6" แทน "12" การ trace context จริง
พบว่า retrieval **ดึงหน้า 15 มาแล้ว** (หน้าที่มีคำตอบ) แต่คำตอบยังผิดเพราะสองเหตุซ้อน:

1. `build_context` ตัดข้อความแต่ละ hit ที่ 500 ตัวอักษร แต่ข้อความ "3) กลุ่มวิชาชีพ
   เฉพาะด้าน 12 หน่วยกิต" อยู่ที่ index 599 ของ chunk2809 → ถูกตัดทิ้ง LLM เห็นแต่
   "6 หน่วยกิต" ที่อยู่ต้น chunk
2. `_dedupe_by_page` เก็บ chunk เดียวต่อหน้า จึงทิ้ง chunk2810 ที่ขึ้นต้นด้วย
   "12 หน่วยกิต" ชัดเจน

**การแก้ (อยู่ในชั้น query ล้วน ไม่แตะ index/embedding):** เพิ่ม
`_expand_page_neighbors(conn, hits)` ที่หลัง cutoff ดึง chunk อื่นในหน้าเดียวกัน
(ข้าม boilerplate) มาเก็บใน field ใหม่ `EvidenceHit.neighbor_text` แล้ว `build_context`
ต่อ main + neighbor เข้าด้วยกันและยกเพดานตัดเป็น 900 ตัวอักษร citation ไม่เปลี่ยน
(ยังหนึ่งหน้าต่อหนึ่ง citation) โดย consistency, ocr field_macro, retrieval hit
เท่าเดิมทุกตัว

**ผลกับ H5 — สิ่งที่การแก้นี้ทำได้จริง และข้อจำกัดที่ตรงไปตรงมา:** ก่อนแก้ context
ที่ส่งเข้า LLM มีแค่ "6 หน่วยกิต" (ส่วนที่มี "12" ถูกตัดทิ้ง) จึงตอบ 6 ได้อย่างเดียว
หลังแก้ context มีครบทั้ง 12 (กลุ่มวิชาชีพเฉพาะด้าน) และ 6 (กลุ่มการศึกษาทางเลือก)
— นี่คือสิ่งที่ retrieval ควบคุมได้และทำสำเร็จ

**แต่คำตอบ H5 ยังแกว่งระหว่างรอบ** (auto_pass จึงอยู่ที่ 18–19/19 ขึ้นกับ LLM)
เพราะคำถามใช้คำว่า "หมวดวิชาเฉพาะเลือก" ซึ่ง **ไม่ตรงเป๊ะกับคำใดในเอกสาร** เอกสาร
เขียน "กลุ่มวิชาชีพเฉพาะด้าน" (12) กับ "กลุ่มวิชาการศึกษาทางเลือก" (6) LLM จึงสับสน
สองกลุ่มนี้เป็นบางรอบ นี่เป็นความกำกวมเชิงศัพท์ที่อยู่ในชั้น generation ไม่ใช่ retrieval

**จงใจไม่แก้ด้วยการ optimize prompt เฉพาะ H5** เพราะจะเป็น workaround ที่ผูกกับ
คำถามข้อเดียว เปราะต่อการแกว่งของ LLM และเสี่ยงกระทบข้ออื่น — บันทึกไว้เป็นข้อจำกัด
ที่รู้ตัว: retrieval ส่งข้อมูลครบแล้ว การตีความคำกำกวมที่เหลือขึ้นกับ LLM

### ขั้น 1B (ปกป้อง lexical top-1 จาก cutoff) — ประเมินแล้ว *จงใจไม่ทำ*

แผนเดิมมีขั้นการันตีว่าหน้าที่ bm25 จัดอันดับ 1 ไม่ถูก adaptive cutoff ตัดทิ้ง
เหตุผลเดิมผูกกับคำถาม "IT 2565 มีกี่แขนงวิชา" แต่ตรวจแล้วพบว่า **คำถามนั้นไม่มีใน
ชุดทดสอบ 19 ข้อจริง** (E3 จริงคือ "การสร้างคลังข้อมูลเรียนปีไหน" ซึ่งตอบถูก)

ไล่ดูทั้ง 19 ข้อหลังแก้ 1A ไม่มีข้อใดที่อาการตรงกับ 1B (lexical จัดอันดับ 1 แล้วถูก
cutoff กลบ) เลย — ข้อที่ยังตัวเลขต่ำคือ H1 (คำถามกว้าง gold กว้างตาม), H2 (recall
1.0 อยู่แล้ว อ้างหน้าเกินเฉย ๆ), H4 (cross-version ข้างต้น) ทั้งสามไม่เกี่ยว cutoff

จึง **ไม่แตะ `_apply_cutoff`** เพราะไม่มีเคสจริงมารองรับ และการปรับ fusion/cutoff
เสี่ยงทำ citation precision ของข้ออื่นตกโดยไม่ได้อะไรคืน — ตัวอย่างของการตัดสินใจ
บนตัวเลข ไม่ใช่ไล่ทำ metric ให้สวย

---

## ภาคผนวก 4: AITBA โท/เอก — สมมติฐานเดิมผิด (ก.ย. 2569)

แผนเดิมระบุว่า AITBA มี 2 เอกสาร (โท+เอก) ผูก `version_id` เดียวกันเพราะไม่ได้
ใช้ `degree_level` แยก version ตรวจแล้วพบว่าสมมติฐานนี้ผิดตั้งแต่ต้น:

- `curriculum_version` ไม่มีคอลัมน์ `degree_level` เลย (อยู่ที่ `document` เท่านั้น)
- `degree_level` ที่ระบบตรวจ **ถูกต้องเสมอ** — อ่านจากเนื้อหาไฟล์ไม่ใช่ชื่อไฟล์
  (ยืนยันด้วย `PH_D_IT2561/2566` ที่ตรวจถูกเป็น `doctoral` ปกติ)

**ต้นเหตุจริง:** ไฟล์ `Doctorals_Degree/PH_D_AITBA2569_current.pdf` กับ
`Masters_Degree/M_AITBA2569_current.pdf` มี **sha256 เหมือนกันเป๊ะ 100%**
และหน้า 1 ของทั้งคู่เขียนว่า "หลักสูตรวิทยาศาสตรมหาบัณฑิต" (ปริญญาโท) เหมือนกัน
ระบบจึงระบุ `degree_level='master'` ให้ทั้งคู่ถูกต้องแล้วตามเนื้อหาจริง — ไฟล์
ต้นฉบับตั้งชื่อผิด (เอาไฟล์ปริญญาโทไปใส่ในโฟลเดอร์ `Doctorals_Degree`) หรือ
มหาลัยไม่มีเอกสารปริญญาเอกของ AITBA ให้ ingest เลย **ไม่ใช่บั๊กของระบบเรา**

ระบบ dedup (`canonical_document_id`) ตรวจจับไฟล์ซ้ำถูกต้องแล้วด้วย sha256
(กลไกที่ถูกกว่าเชื่อชื่อไฟล์) แต่ยัง insert chunk สองชุดจากเนื้อหาเดียวกัน
(1606 chunk / embedding ควรมีแค่ ~800) — ตรวจแล้วว่า**ตาราง `course` ไม่มีรหัส
ซ้ำเลย** (0/46) เพราะมี dedup ของตัวเองอยู่แล้วในชั้น populate ผลกระทบที่เหลือ
จึงเป็นเรื่อง**ประสิทธิภาพ** (เปลืองพื้นที่/เวลา embed สองเท่า) ไม่ใช่ความถูกต้อง
ของคำตอบ

**จงใจไม่แก้ในรอบนี้** เพราะการลบ chunk/embedding ซ้ำต้องลบข้อมูลจากฐานจริง
(destructive) เพื่อแก้ปัญหาประสิทธิภาพที่ไม่กระทบความถูกต้องคำตอบ และ AITBA
ไม่มีคำถามอยู่ในชุดทดสอบ 19 ข้อเลย จึงไม่มีตัวเลขหลักตัวใดได้ประโยชน์จากการแก้นี้
